...
def main():
    ...
    print("Loading PPO V2.3...")

    model = PPO.load(
        "/workspace/ai-fight-arena/training/alpha_policy_v23.zip",
        device="cpu"
    )

    print("PPO V2.3 loaded.")
    ...
    print("========================================")
    print("AI FIGHT ARENA - ISAAC SIM PPO V2.3")
    ...
    while simulation_app.is_running():
        ...
        if winner is not None:
            ...
            alpha_hp_sum += alpha_health
            bravo_hp_sum += bravo_health
            decision_sum += fight_decisions

            print()
            print(
                f"FIGHT {completed_fights:02d}/{total_fights} | "
                f"Winner={winner} | "
                f"Alpha HP={alpha_health:.1f} | "
                f"Bravo HP={bravo_health:.1f} | "
                f"Decisions={fight_decisions}"
            )
            print()
    ...
    print()
    print("=" * 60)
    print("ISAAC SIM PPO V2.3 EVALUATION")
    print("=" * 60)
    print(f"Fights:                 {completed_fights}")
    ...
    if completed_fights > 0:
        print(
            f"Alpha win rate:         "
            f"{100.0 * alpha_wins / completed_fights:.1f}%"
        )
        print(
            f"Average Alpha HP:       "
            f"{alpha_hp_sum / completed_fights:.1f}"
        )
        print(
            f"Average Bravo HP:       "
            f"{bravo_hp_sum / completed_fights:.1f}"
        )
        print(
            f"Average fight length:   "
            f"{decision_sum / completed_fights:.1f} decisions"
        )

    print(f"Alpha attacks:          {total_attacks}")
    print(f"Alpha blocks:           {total_blocks}")
    print(f"Alpha dodges:           {total_dodges}")
    print(f"Bravo telegraphs:       {total_telegraphs}")
    print("=" * 60)
